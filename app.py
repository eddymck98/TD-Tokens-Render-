...
def send_verification_email(to_email, verification_link):
  try:
    ...
    st.error(f"Failed to send verification email: {e}"); return False
...
if "type" in query_params and query_params["type"] == "recovery":
  ...
  try:
    ...
    st.session_state["is_password_recovery"] = True; st.query_params.clear()
  except Exception: pass
  ...
  try:
    ...
    if session_cookie:
      ...
      if isinstance(session_cookie, dict):
        ...
        if session_cookie.startswith("{"): token_data = json.loads(session_cookie); acc_token, ref_token = token_data.get("access_token"), token_data.get("refresh_token")
        else: acc_token = ref_token = session_cookie
      if acc_token and ref_token:
        res = supabase.auth.set_session(acc_token, ref_token)
        if res and res.user:
          st.session_state.user = res.user
          if res.session: controller.set("td_tokens_session", json.dumps({"access_token": res.session.access_token, "refresh_token": res.session.refresh_token}), max_age=2592000)
          st.rerun()
  except Exception: pass
...
def get_true_global_token_balance(target_user_id):
  try: return max(0, (supabase.table("profiles").select("tokens").eq("id", target_user_id).single().execute().data or {}).get("tokens", 10))
...
NFL_TEAM_DATA = get_static_nfl_team_data(); NFL_TEAMS = list(NFL_TEAM_DATA.keys())
...
MASTER_BADGES = {"🚀 Token Tycoon": "Accumulate 50+ lifetime tokens earned across your career", "🎯 High Roller": "Wager 10+ tokens on a single question", "⚡ Double Down Legend": "Wager 15+ total tokens in a single week", "💣 All-In Maverick": "Wager 100% of your remaining token balance on a slate", "🏈 TD Guru": "Correctly predict 5+ Touchdown Scorers", "🎯 Sniper": "Correctly predict 9+ Touchdown Scorers across the season", "👑 Weekly High Scorer": "Win the most net tokens in a single week", "🎯 Perfect 10/10": "Correctly answer all 10 scenarios in a single week", "🧊 Clutch Gene": "Win a scenario where 75%+ of the league picked the wrong side", "🛡️ Iron Defender": "Submit bets for 5 or more weeks without missing", "💰 Century Club": "Accumulate 100+ total cumulative tokens won across history", "📉 Wall Street Bets": "Take the largest token loss in a single week", "📉 Down Bad": "Reach a token balance of 0 tokens", "🏆 League Champion": "Be crowned the official end-of-season League Champion", "⭐ League Commissioner": "Create or administer a custom mini-league", "🔮 Oracle of Delphi": "Successfully call a 5+ token wager correctly 4 weeks in a row", "🔥 Untouchable Run": "Gain 20+ net tokens in a single weekly slate", "⚡ Gridiron Prophet": "Correctly predict 13+ Touchdown Scorers across the season", "💎 Diamond Hands": "Survive with fewer than 3 tokens remaining and bounce back to 30+", "🛡️ Mini-League Monarch": "Finish 1st place in any active mini-league standing", "🌟 Gridiron General": "Maintain a 70%+ win rate across 20+ total bets in your mini-league", "🎯 Pick Six Prodigy": "Correctly predict a Touchdown Scorer on 3 consecutive weeks", "🎩 Commissioner's Right Hand": "Be an active member of 3+ custom mini-leagues"}
AVAILABLE_TITLES = {"🏈 Gridiron Contender": {"badge": None, "req": "Default baseline title for all players."}, "👑 League Champion": {"badge": "🏆 League Champion", "req": "Be crowned the official end-of-season League Champion."}, "⭐ League Commissioner": {"badge": "⭐ League Commissioner", "req": "Create or administer a custom mini-league."}, "🔮 The Oracle": {"badge": "🔮 Oracle of Delphi", "req": "Successfully call a 5+ token wager correctly 4 weeks in a row."}, "💰 Token Tycoon": {"badge": "🚀 Token Tycoon", "req": "Accumulate 50+ lifetime tokens earned."}, "⚡ Gridiron Prophet": {"badge": "⚡ Gridiron Prophet", "req": "Correctly predict 13+ Touchdown Scorers across the season."}, "🎯 Sharp Shooter": {"badge": "🎯 Sniper", "req": "Correctly predict 9+ Touchdown Scorers across the season."}, "🏈 TD Specialist": {"badge": "🏈 TD Guru", "req": "Correctly predict 5+ Touchdown Scorers."}, "🛡️ Mini-League Monarch": {"badge": "🛡️ Mini-League Monarch", "req": "Finish in 1st place in any active mini-league."}, "📉 Bankrupt Gambler": {"badge": "📉 Down Bad", "req": "Reach a token balance of 0 tokens."}}
DEFAULT_QUESTION_TEMPLATES = ["Will QB 1 throw for over 250+ passing yards?", "Will RB 1 rush for 75+ rushing yards?", "Will WR 1 catch 6 or more receptions?", "Will Away Team score a touchdown in the 1st quarter?", "Will there be a successful 50+ yard Field Goal kicked?", "Will this game have over 45.5 combined points scored?", "Will any Defense record a pick-six or fumble recovery touchdown?", "Will TE 1 score a rushing or receiving touchdown?", "Will this game go into Overtime?", "Will Home Team record 3 or more sacks?"]
...
if st.session_state.user:
  try:
    ...
    if res.data: t_info = NFL_TEAM_DATA.get(res.data.get("favorite_team", "🏈 Free Agent / Neutral"), NFL_TEAM_DATA["🏈 Free Agent / Neutral"]); user_team_color, user_team_logo, user_stadium_bg = t_info["color"], t_info["logo"], t_info["stadium"]
  except Exception: pass
...
st.markdown(f"""
    ...
    .matchup-card-item {{ background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255, 255, 255, 0.12); border-left: 4px solid {user_team_color}; border-radius: 14px; padding: 16px 20px; margin-bottom: 12px; backdrop-filter: blur(14px); box-shadow: 0 6px 20px rgba(0,0,0,0.35); transition: transform 0.2s ease; }}
    .matchup-card-item:hover {{ transform: translateY(-2px); border-color: rgba(255, 255, 255, 0.25); }}
...
def sync_and_get_user_badges(target_user_id, check_celebration=False):
  ...
  if len(u_td) >= 5: newly_earned.add("🏈 TD Guru")
  if len(u_td) >= 9: newly_earned.add("🎯 Sniper")
  if len(u_td) >= 13: newly_earned.add("⚡ Gridiron Prophet")
  ...
  if len(joined_l_ids) >= 3: newly_earned.add("🎩 Commissioner's Right Hand")

  if u_td:
    correct_weeks = sorted([td["week_number"] for td in u_td])
    consec_count, max_consec = 1, 1
    for i in range(1, len(correct_weeks)):
      if correct_weeks[i] == correct_weeks[i - 1] + 1: consec_count += 1; max_consec = max(max_consec, consec_count)
      else: consec_count = 1
    if max_consec >= 3: newly_earned.add("🎯 Pick Six Prodigy")
  ...
  for b in u_bets:
    ...
    if w_ans in ["Yes", "No"]:
      if b["pick"] == w_ans:
        total_lifetime_won += b["wager_amount"]; weekly_nets[w_num]["gains"] += b["wager_amount"]
  ...
  if total_lifetime_won >= 50: newly_earned.add("🚀 Token Tycoon")

  sorted_weeks, consecutive_oracle_weeks = sorted(list(weekly_nets.keys())), 0
  for w in sorted_weeks:
    if any(b["wager_amount"] >= 5 and b["pick"] == b.get("weekly_questions", {}).get("winning_answer") for b in [b for b in u_bets if b["week_number"] == w]):
      consecutive_oracle_weeks += 1
      if consecutive_oracle_weeks >= 4: newly_earned.add("🔮 Oracle of Delphi")
    else: consecutive_oracle_weeks = 0
  ...
  for l_id in joined_l_ids:
    if l_id == "00000000-0000-0000-0000-000000000001": continue
    ...
    if peer_ids:
      ...
      if mini_stats:
        if mini_stats[0]["id"] == target_user_id: newly_earned.add("🛡️ Mini-League Monarch")
        my_mini_stat = next((s for s in mini_stats if s["id"] == target_user_id), None)
        if my_mini_stat and my_mini_stat["win_rate"] >= 70 and my_mini_stat["total_bets"] >= 20: newly_earned.add("🌟 Gridiron General")
...
def calculate_nemesis(target_user_id, allowed_peer_ids=None):
  ...
  except Exception: return "None Yet", 0
...
def get_cached_leaderboard_stats(allowed_peer_ids=None, selected_league_id=None):
  ...
  for p in leader_res:
    ...
    stats.append({**p, "tokens": true_global_tokens, "correct_tds": len(correct_tds) if correct_tds else 0, "win_rate": int((wins / total_graded) * 100) if total_graded > 0 else 0, "total_bets": total_graded, "nemesis_name": nem_name, "nemesis_score": nem_score, "streak": calculate_streak(p["id"])})
...
def recalculate_all_user_balances(supabase_client):
  try:
    ...
    for prof in all_profiles:
      ...
      td_wins_map, curr_tokens = {td["week_number"]: 5 for td in u_td}, 10
...
try: is_signin_locked = (supabase.table("weekly_questions").select("winning_answer").eq("week_number", 998).execute().data or [{"winning_answer": ""}])[0]["winning_answer"] == "LOCKED"
except Exception: pass
try: is_signup_locked = (supabase.table("weekly_questions").select("winning_answer").eq("week_number", 997).execute().data or [{"winning_answer": ""}])[0]["winning_answer"] == "LOCKED"
except Exception: pass
...
if st.session_state.get("is_password_recovery", False):
  ...
  with st.form("password_recovery_screen_form"):
    ...
    if submit_new_pass:
      if len(new_p1) < 6: st.warning("Password must be at least 6 characters long.")
      elif new_p1 != new_p2: st.error("Passwords do not match.")
      elif not recovery_email.strip(): st.warning("Please enter your email address.")
      else:
        try:
          token_val = st.query_params.get("token", "")
          if token_val:
            res = supabase.auth.verify_otp({"email": recovery_email.strip(), "token": token_val, "type": "recovery"})
            if res and res.session: supabase.auth.set_session(res.session.access_token, res.session.refresh_token)
          supabase.auth.update_user({"password": new_p1})
          st.session_state["is_password_recovery"] = False; st.query_params.clear()
          st.success("Password updated successfully! You can now log in."); st.rerun()
        except Exception as e: st.error(f"Failed to update password: {e}")

  if st.button("Cancel & Return to Login"): st.session_state["is_password_recovery"] = False; st.query_params.clear(); st.rerun()
...
# ==========================================
  ...
  if st.session_state.get("signup_success_email"):
    ...
    with tab_login:
      ...
      if is_signin_locked: st.error("🔒 **SIGN-IN LOCKED:** The Admin has temporarily disabled log-ins. Please check back soon!")
      else:
          ...
          if login_email and login_password:
            try:
              ...
              if user and user.email_confirmed_at:
                ...
                if auth_response.session: controller.set("td_tokens_session", json.dumps({"access_token": auth_response.session.access_token, "refresh_token": auth_response.session.refresh_token}), max_age=2592000)
                st.success("Successfully logged in!"); st.rerun()
              else: supabase.auth.sign_out(); st.error("Please authorise your email first before logging in. Check your inbox for the verification link.")
            except Exception: st.error("Invalid login credentials or unverified account. Please authorise first.")
            ...
            if reset_email:
              try:
                ...
                if response and hasattr(response, "properties") and response.properties:
                  ...
                  if recovery_link:
                    html_content = f"""<div style="background-color: #0b0f19; padding: 30px; font-family: 'Inter', Arial, sans-serif; color: #f8fafc;"><div style="max-width: 600px; margin: 0 auto; background: rgba(15, 23, 42, 0.95); border: 1px solid rgba(255, 255, 255, 0.12); border-top: 4px solid #fbbf24; border-radius: 16px; padding: 40px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);"><div style="text-align: center; margin-bottom: 30px;"><img src="https://github.com/eddymck98/TD-Tokens-Render-/blob/main/TD%20Tokens%207.png?raw=true" alt="Touchdown Tokens Logo" style="width: 180px; margin-bottom: 15px; filter: drop-shadow(0px 6px 15px rgba(251, 191, 36, 0.4));" /><h1 style="font-family: 'Bebas Neue', Arial, sans-serif; color: #fbbf24; font-size: 32px; letter-spacing: 2px; margin: 0;">TOUCHDOWN TOKENS</h1><p style="color: #93c5fd; font-size: 14px; letter-spacing: 3px; text-transform: uppercase; margin-top: 5px;">Password Reset Request</p></div><h3 style="color: #ffffff; font-size: 20px; margin-bottom: 15px;">Reset Your Password 🔑</h3><p style="color: #cbd5e1; font-size: 15px; line-height: 1.6; margin-bottom: 25px;">We received a request to reset your Touchdown Tokens password. Click the secure button below to choose a brand new password for your account:</p><div style="text-align: center; margin: 35px 0;"><a href="{recovery_link}" style="background: linear-gradient(135deg, #fbbf24 0%, #d97706 100%); color: #000000; padding: 14px 28px; text-decoration: none; border-radius: 12px; font-weight: bold; font-size: 16px; letter-spacing: 1px; display: inline-block; box-shadow: 0 6px 20px rgba(251, 191, 36, 0.3);">RESET PASSWORD</a></div><p style="color: #94a3b8; font-size: 13px; line-height: 1.5; margin-top: 30px; border-top: 1px solid rgba(255, 255, 255, 0.08); padding-top: 20px;">If you did not request a password reset, you can safely ignore and delete this email. Your account remains completely secure.</p></div><div style="text-align: center; margin-top: 20px; color: #64748b; font-size: 12px;">&copy; 2026 Touchdown Tokens. All rights reserved.</div></div>"""
                    resend.Emails.send({"from": "Touchdown Tokens <noreply@auth.tdtokens.co.uk>", "to": [reset_email.strip()], "subject": "🔑 Reset Your Touchdown Tokens Password", "html": html_content})
                    st.success("Password reset email sent via Resend! Check your inbox.")
                  else: st.error("Could not retrieve recovery link properties from the response.")
                else: st.error("Could not generate recovery link for this email.")
    ...
    with tab_signup:
      ...
      if is_signup_locked: st.error("🔒 **SIGN-UP LOCKED:** The Admin has temporarily disabled new account registrations. Please check back soon!")
      else:
        st.caption("New players start with 10 free tokens!")
        col_fn, col_sn = st.columns(2)
        with col_fn: reg_first_name = st.text_input("First Name", key="reg_first_name")
        with col_sn: reg_surname = st.text_input("Surname", key="reg_surname")
        ...
        if st.button("Sign Up"):
          if not reg_first_name.strip(): st.warning("Please enter your first name.")
          elif not reg_surname.strip(): st.warning("Please enter your surname.")
          elif not signup_email.strip(): st.warning("Please enter your email address.")
          elif not tc_accepted: st.warning("You must accept the Terms of Service & User Agreement to create an account.")
          else:
            combined_full_name = f"{reg_first_name.strip()} {reg_surname.strip()}"
            if contains_profanity(combined_full_name): st.error("⚠️ Your name contains restricted language. Please choose appropriate wording.")
            else:
              try:
                ...
                if response.user:
                  ...
                  st.session_state.signup_success_email = signup_email.strip(); st.rerun()
...
# ==========================================
  ...
  st.markdown(f"""<div class="sticky-balance-bar"><div style="display: flex; align-items: center; gap: 14px;"><div style="border: 3px {user_border_style} {user_team_color}; border-radius: 10px; padding: 3px 8px; background: {user_avatar_color}; box-shadow: 0 4px 12px {user_team_color}33;"><span style="font-size: 26px;">{user_avatar}</span></div><div><b style="font-size: 16px; color: #ffffff;">{profile['full_name']}</b> <span style="font-size:11px; color:#38bdf8; margin-left:6px;">({get_earned_title(user_id)})</span><div style="font-size: 11px; color: #94a3b8; text-transform: uppercase; font-weight: 600;">{user_team}</div></div></div><div style="display: flex; align-items: center; gap: 15px;"><div style="text-align: right;"><span style="font-family: 'Bebas Neue'; font-size: 26px; color: {user_team_color};">{active_tokens_display} 🪙</span><div style="font-size: 10px; color: #94a3b8; text-transform: uppercase; font-weight: 600;">Available Tokens</div></div></div></div>""", unsafe_allow_html=True)

  tabs_options = ["🏠 Home", "👤 Profile", "📖 Rules", "🎯 Bets", "📜 History", "🛡️ Leagues", "⭐ Commish", "⚙️ Settings", "🛠️ Admin"]
  if profile.get("is_admin") and is_any_league_admin: tab_home, tab_profile, tab_rules, tab_bet, tab_history, tab_leagues, tab_league_admin, tab_settings, tab_admin = st.tabs(tabs_options)
  elif profile.get("is_admin"): tab_home, tab_profile, tab_rules, tab_bet, tab_history, tab_leagues, tab_settings, tab_admin = st.tabs([t for t in tabs_options if t != "⭐ Commish"])
  elif is_any_league_admin: tab_home, tab_profile, tab_rules, tab_bet, tab_history, tab_leagues, tab_league_admin, tab_settings = st.tabs([t for t in tabs_options if t != "🛠️ Admin"])
  else: tab_home, tab_profile, tab_rules, tab_bet, tab_history, tab_leagues, tab_settings = st.tabs([t for t in tabs_options if t not in ["⭐ Commish", "🛠️ Admin"]])

  with tab_home:
    st.markdown(f"""<div class="hero-banner"><div><div style="font-size: 14px; letter-spacing: 2px; text-transform: uppercase; color: #93c5fd; font-weight: 600;">Available Balance</div><div class="hero-tokens-val">{active_tokens_display} 🪙</div><div style="font-size: 13px; color: #cbd5e1; margin-top: 4px;">True Global Bank: <b>{true_global_tokens_sidebar} 🪙</b> (Active Wagers Deducted)</div></div><div style="text-align: right; border-left: 1px solid rgba(255,255,255,0.15); padding-left: 20px;"><div style="font-size: 12px; color: #94a3b8; text-transform: uppercase; font-weight: 700;">Welcome Back</div><div style="font-size: 18px; font-weight: bold; color: #ffffff;">{profile['full_name']}</div><div style="font-size: 11px; color: #38bdf8;">{get_earned_title(user_id)}</div></div></div>""", unsafe_allow_html=True)
    ...
    if not available_weeks: st.info("No active weeks available.")
      ...
      if not curr_user_bets and not curr_user_td: st.warning(f"You haven't submitted any picks for Week {view_week} yet! Head over to the 'Place Bets' tab.")
      else:
        ...
        for b in curr_user_bets:
          ...
          if w_ans in ["Yes", "No"]: card_border_glow, card_shadow_glow, status_label = ("#10b981", "rgba(16, 185, 129, 0.3)", "Won ✅") if pick_val == w_ans else ("#ef4444", "rgba(239, 68, 68, 0.3)", "Lost ❌")
          else: card_border_glow, card_shadow_glow, status_label = "#38bdf8", "rgba(56, 189, 248, 0.2)", "Pending ⏳"
        ...
        st.write(""); st.subheader("📋 Group Chat Share Text"); st.code("\n".join(share_lines), language="markdown"); st.success("Copy the text box above to share your picks directly into WhatsApp or group chat!")
    ...
    if available_weeks:
      ...
      if live_all_bets:
        ...
        for b in live_all_bets:
          ...
          if clean_q not in q_stats: q_stats[clean_q] = {"Yes": 0, "No": 0, "TotalWager": 0, "Votes": 0}
          q_stats[clean_q][b["pick"]] += 1; q_stats[clean_q]["TotalWager"] += b["wager_amount"]; q_stats[clean_q]["Votes"] += 1
        ...
        else: st.info("No bets placed for the current week yet. Be the first to lock in your picks!")
      else: st.info("No bets placed for the current week yet. Be the first to lock in your picks!")

    st.divider(); st.subheader("📊 Last Week's Performance Summary")
    ...
    if closed_markers: [graded_weeks_set.add(cm["week_number"]) for cm in closed_markers]
    if all_graded_weeks_meta:
      w_map = {}
      for q in all_graded_weeks_meta:
        if q["week_number"] not in w_map: w_map[q["week_number"]] = []
        if q.get("question_number", 0) <= 10: w_map[q["week_number"]].append(q["winning_answer"])
    ...
    if not graded_weeks_list: st.info("No weeks have been graded yet. Place your bets for Week 1 to get started!")
    else:
      ...
      if not lw_bets and not lw_td: st.warning(f"You did not submit any bets or touchdown picks for Week {latest_graded_week}.")
      else:
        bet_gains, bet_losses, correct_count, total_bets_placed = 0, 0, 0, len(lw_bets)
        for b in lw_bets:
          w_ans = b.get("weekly_questions", {}).get("winning_answer")
          if w_ans in ["Yes", "No"]:
            if b["pick"] == w_ans: bet_gains += b["wager_amount"]; correct_count += 1
            else: bet_losses += b["wager_amount"]
        td_record = lw_td[0] if lw_td else None
        if td_record is None or td_record.get("is_correct") is None: td_is_graded, td_bonus, td_display_status = False, 0, "⏳ Pending (Awaiting Admin Grading)"
        else: td_is_graded, td_bonus, td_display_status = True, 5 if str(td_record.get("is_correct")).lower() == "true" else 0, "✅ Correct (+5 Tokens)" if str(td_record.get("is_correct")).lower() == "true" else "❌ Incorrect (Missed)"
        ...
        if net_total > 0 and not st.session_state.get(celeb_key, False): st.balloons(); st.session_state[celeb_key] = True
  ...
  with tab_profile:
    st.header("👤 Profile & Customization Hub"); st.caption("Personalize your display avatar, title nametag, border style, avatar color, favorite player, favorite team, and featured badges!")
    ...
    with col_logo: st.image(selected_team_data["logo"], width=75)
    with col_info: st.markdown(f"### {new_team}")
    ...
    for title_name, info in AVAILABLE_TITLES.items():
      if info["badge"] is None or info["badge"] in user_badges_for_titles: unlocked_title_options.append(title_name)
      else: locked_title_info.append((title_name, info["req"]))
    
    curr_selected_title = profile.get("selected_title", "🏈 Gridiron Contender")
    if curr_selected_title not in unlocked_title_options: curr_selected_title = unlocked_title_options[0] if unlocked_title_options else "🏈 Gridiron Contender"
    
    with st.form("profile_customization_form"):
      new_display_name = st.text_input("Display Name", value=profile.get("full_name", ""))
      col_t1, col_t2 = st.columns(2)
      with col_t1: new_title = st.selectbox("Active Nametag Title", unlocked_title_options, index=unlocked_title_options.index(curr_selected_title) if curr_selected_title in unlocked_title_options else 0, help="Select from your unlocked prestigious titles!")
      with col_t2:
      ...
      with col_av3: new_av_color = st.color_picker("Avatar Box Color", value=profile.get("avatar_color", "#1e3a8a"))
      ...
      if st.form_submit_button("Save Profile Settings 💾", type="primary"):
        ...
        elif contains_profanity(new_display_name) or contains_profanity(new_fav_player) or contains_profanity(new_bio): st.error("⚠️ Your profile input contains restricted language. Please choose appropriate wording.")
        else:
    ...
    st.divider(); st.subheader("⭐ Featured Badge Showcase"); st.caption("Choose up to 3 unlocked badges to showcase on your leaderboard card.")
    ...
    with st.form("featured_badges_form"):
      ...
      if st.form_submit_button("Save Featured Badges 🌟", type="primary"): supabase.table("profiles").update({"featured_badges": selected_featured}).eq("id", user_id).execute(); st.success("Featured badges updated successfully!"); st.rerun()

    st.divider(); st.subheader("🏆 Virtual Trophy Cabinet"); st.caption("Inspect badge showcases across any league member.")
    all_league_profiles = get_cached_profiles(); user_name_map = {p["full_name"]: p for p in all_league_profiles}
    ...
    with col_t_logo: st.image(selected_team_info["logo"], width=70)
    with col_t_info: st.markdown(f"### {selected_player.get('avatar_emoji', '🏈')} {selected_player['full_name']}'s Showcase"); st.markdown(f"**Unlocked:** `{len(selected_badges)}` / `{len(MASTER_BADGES)}` Badges")
    st.progress(progress_ratio, text=f"**Cabinet Completion:** `{int(progress_ratio * 100)}%` Unlocked"); st.write("")

    t_col1, t_col2 = st.columns(2)
    for idx, (b_name, b_desc) in enumerate(MASTER_BADGES.items()):
      with (t_col1 if idx % 2 == 0 else t_col2):
        if b_name in selected_badges: st.markdown(f"""<div class="trophy-card-unlocked"><b>{b_name}</b> <span style="color:#fbbf24; font-weight:bold;">(UNLOCKED)</span><br><small style="color:#cbd5e1;">{b_desc}</small></div>""", unsafe_allow_html=True)
    ...
    if not available_weeks: st.info("No active questions available yet. Check back soon when the Admin posts Week 1!")
      ...
      for w in available_weeks:
        ...
        if not is_closed:
          ...
          if w_qs_check and all(q["winning_answer"] in ["Yes", "No"] for q in w_qs_check): is_closed = True
        if not is_closed: active_unscored_weeks.append(w)
      
      if not active_unscored_weeks: st.info("🎉 All currently available weeks have been graded and closed! Check back when the Admin posts a new active week.")
      else:
        ...
        if lock_time_row:
          try:
            ...
            if total_seconds_left <= 0: is_locked = True; st.error("🔒 Entries for this week are locked! Kickoff deadline has passed.")
            else:
              days, remainder = divmod(total_seconds_left, 86400); hours, remainder = divmod(remainder, 3600); minutes, seconds = divmod(remainder, 60)
        ...
        if any(q.get("winning_answer") == "LOCKED" for q in questions): is_locked = True; st.error("🔒 Entries for this week have been manually locked by the Admin.")
        ...
        if not questions: st.info("No questions found for this week.")
              ...
              if st.button("🎲 Feeling Lucky (Randomize)", help="Randomly distributes your available tokens and picks across the questions!"):
                with st.spinner("🎲 Simulating lucky picks and distributing tokens..."):
                  ...
                  if real_q_items:
                    ...
                    for _ in range(true_global_tokens_bet): token_allocations[random.choice(real_q_items)["id"]] += 1
                    for q_item in real_q_items: supabase.table("user_bets").insert({"user_id": user_id, "user_name": profile["full_name"], "week_number": selected_week, "question_id": q_item["id"], "pick": random.choice(["Yes", "No"]), "wager_amount": token_allocations[q_item["id"]]}).execute()
                    st.cache_data.clear(); st.session_state.form_refresh += 1; st.success("🎲 Random bets generated and populated successfully!"); st.rerun()
          ...
          with st.form("weekly_bet_form"):
            ...
            for q in questions:
              ...
              full_q_text = q["question_text"]; away_team_name = home_team_name = "🏈 Free Agent / Neutral"; prompt_text = full_q_text
              if " | MATCHUP: " in full_q_text:
                prompt_text, matchup_str = full_q_text.split(" | MATCHUP: ")
                if " @ " in matchup_str: away_team_name, home_team_name = matchup_str.split(" @ ")
              ...
              with st.expander(f"Matchup Q{q['question_number']}: {away_team_name} @ {home_team_name} (Wager: {default_wager_val} 🪙)", expanded=True):
                ...
                with col_pick: picks[q["id"]] = st.radio(f"Pick Q{q['question_number']}", ["Yes", "No"], index=0 if default_pick_val == "Yes" else 1, key=f"pick_w{selected_week}_{q['id']}_{st.session_state.form_refresh}", horizontal=True, disabled=is_locked, label_visibility="collapsed")
                with col_wager: wagers[q["id"]] = st.number_input(f"Wager Q{q['question_number']} Tokens", min_value=0, max_value=true_global_tokens_bet, value=default_wager_val, key=f"wager_w{selected_week}_{q['id']}_{st.session_state.form_refresh}", disabled=is_locked)
            ...
            if total_wagered > true_global_tokens_bet: st.error(f"⚠️ Over-wagered! You have allocated {total_wagered} tokens but only have {true_global_tokens_bet} available.")
            else: st.progress(min(1.0, total_wagered / max(1, true_global_tokens_bet)), text=f"**Tokens Allocated:** `{total_wagered}` / `{true_global_tokens_bet}` Tokens ({int(min(1.0, total_wagered / max(1, true_global_tokens_bet)) * 100)}%)")
            ...
            with col_sub1: submit_bet = st.form_submit_button("Submit Weekly Bets 🚀", type="primary", disabled=is_locked)
            with col_sub2: clear_bet = st.form_submit_button("Clear Bet Choices 🗑️", disabled=is_locked)
            ...
            if clear_bet and not is_locked:
              ...
              st.session_state.form_refresh += 1; st.success("Your bet choices for this week have been cleared!"); st.rerun()

            if submit_bet and not is_locked:
              if contains_profanity(td_pick): st.error("⚠️ Your Touchdown Scorer player pick contains restricted language. Please choose a valid player name.")
              elif total_wagered > true_global_tokens_bet: st.error(f"Cannot wager {total_wagered} tokens! You only have {true_global_tokens_bet} tokens available.")
              else:
                ...
                st.balloons(); st.success("Your bets and touchdown pick have been successfully locked in!")

  with tab_history:
    st.header("📜 Your Past Bets & Results"); st.caption("Review your historical predictions, weekly outcomes, and track your performance over time.")
    ...
    if all_graded_weeks_res:
      ...
      for q in all_graded_weeks_res:
        if q["week_number"] not in week_ans_map: week_ans_map[q["week_number"]] = []
        if q.get("question_number", 0) <= 10: week_ans_map[q["week_number"]].append(q["winning_answer"])
      [graded_weeks_set.add(w) for w, ans_list in week_ans_map.items() if ans_list and all(a in ["Yes", "No"] for a in ans_list)]
    
    graded_weeks_list = sorted(list(graded_weeks_set))
    st.subheader("🏈 Touchdown Scorer Pick History"); st.caption("Review your bonus touchdown scorer pick outcomes week by week.")
    ...
    else: st.info("No touchdown scorer picks submitted yet.")

    st.divider(); st.subheader("📋 Detailed Question Bet History")
    ...
    if not history_bets: st.info("You haven't placed any question bets yet.")
    else:
      user_history_weeks = sorted(list(set([b["week_number"] for b in history_bets])))
      selected_history_week = st.selectbox("Filter History by Week", user_history_weeks, index=len(user_history_weeks) - 1, key="history_week_dropdown_filter"); st.write("")
      filtered_history_bets = [b for b in history_bets if b["week_number"] == selected_history_week]
      if not filtered_history_bets: st.info(f"No bets found for Week {selected_history_week}.")
      else:
        formatted_data = []
    ...
    with st.expander("⚔️ Side-by-Side History Comparison vs. Rival", expanded=False):
      ...
      if rival_options and graded_weeks_list:
        ...
        with col_comp_w: comp_week_sel = st.selectbox("Select Graded Week for Comparison", graded_weeks_list, key="hist_comp_week")
        with col_comp_r: comp_rival_name = st.selectbox("Select Rival (Shared League Member)", list(rival_options.keys()), key="hist_comp_rival")
        ...
        if my_hist_bets:
          st.write(""); rival_team_info = NFL_TEAM_DATA.get(rival_prof.get("favorite_team"), NFL_TEAM_DATA["🏈 Free Agent / Neutral"])
          ...
          for b in my_hist_bets:
            ...
            if riv_pick in ["Yes", "No"]: riv_pill_bg, riv_pill_border, riv_pill_color, riv_status_text = ("rgba(16, 185, 129, 0.18)", "#10b981", "#34d399", f"Won (+{riv_wager}🪙 Net)") if riv_won else ("rgba(239, 68, 68, 0.18)", "#ef4444", "#f87171", f"Lost (-{riv_wager}🪙)")
            else: riv_pill_bg, riv_pill_border, riv_pill_color, riv_status_text = "rgba(100, 116, 139, 0.2)", "#64748b", "#94a3b8", "Did Not Bet"
        ...
        else: st.info("You did not place any bets for this selected comparison week.")
      else: st.info("Side-by-side historical comparison will unlock here automatically once at least one week has been fully graded by the Admin and you share a league with other active participants!")

  with tab_leagues:
    st.header("🏆 League Standings & Mini-Leagues"); st.caption("Track your standings across the Global Leaderboard and your custom mini-leagues. Your Nemesis is tracked exclusively within the selected league!")
    ...
    if next((m for m in all_my_leagues if m["leagues"]["id"] == "00000000-0000-0000-0000-000000000001"), None): league_filter_options["🏆 Global Leaderboard"] = "00000000-0000-0000-0000-000000000001"
    for m in [m for m in all_my_leagues if m["leagues"]["id"] != "00000000-0000-0000-0000-000000000001"]: league_filter_options[f"🛡️ {m['leagues']['league_name']} (Mini-League)"] = m['leagues']['id']
    ...
    if league_filter_options:
      ...
      selected_league_filter_id = league_filter_options[selected_league_filter_label]; st.write("")
      ...
      if not filtered_player_stats: st.info("No players found in this standings view yet.")
      else:
        ...
        for idx, p in enumerate(filtered_player_stats):
          ...
          if not (idx > 0 and score == prev_score and tds == prev_tds): current_rank = idx + 1
          prev_score, prev_tds = score, tds
          all_ranked_players.append((p, current_rank))
        
        if is_global_view:
          for p_obj, r_num in all_ranked_players[:10]: render_player_row(p_obj, r_num)
          logged_in_entry = next((item for item in all_ranked_players if item[0]["id"] == user_id), None)
          if logged_in_entry and logged_in_entry[1] > 10: st.markdown("<div style='text-align:center; color:#94a3b8; margin: 8px 0; font-size: 12px;'>• • • your standing • • •</div>", unsafe_allow_html=True); render_player_row(logged_in_entry[0], logged_in_entry[1])
        else:
          for p_obj, r_num in all_ranked_players: render_player_row(p_obj, r_num)
      ...
      if not is_global_view:
        with st.expander("⚔️ Head-to-Head Player Comparison", expanded=False):
          if filtered_player_stats:
            ...
            if all_other_names:
              ...
              with c1: st.markdown(f"""<div class="vs-card"><h3>{my_stat.get('avatar_emoji', '🏈')} You ({my_stat['full_name']})</h3><h2 style="color: {user_team_color};">{my_stat['tokens']} 🪙</h2><p><b>Title:</b> {get_earned_title(user_id)}</p><p><b>Win Rate:</b> {my_stat['win_rate']}%</p><p><b>Correct TDs:</b> {my_stat['correct_tds']}</p><p><b>Nemesis:</b> <span style="color:#f87171;">{my_stat['nemesis_name']}</span> ({my_stat['nemesis_score']})</p></div>""", unsafe_allow_html=True)
              with c2: st.markdown("<h1 style='text-align:center; margin-top:50px;'>VS</h1>", unsafe_allow_html=True)
              with c3: st.markdown(f"""<div class="vs-card"><h3>{rival_stat.get('avatar_emoji','🏈')} {rival_stat['full_name']}</h3><h2 style="color: {NFL_TEAM_DATA.get(rival_stat.get('favorite_team'), NFL_TEAM_DATA['🏈 Free Agent / Neutral'])['color']};">{rival_stat['tokens']} 🪙</h2><p><b>Title:</b> {get_earned_title(rival_stat['id'])}</p><p><b>Win Rate:</b> {rival_stat['win_rate']}%</p><p><b>Correct TDs:</b> {rival_stat['correct_tds']}</p><p><b>Nemesis:</b> <span style="color:#f87171;">{rival_stat['nemesis_name']}</span> ({rival_stat['nemesis_score']})</p></div>""", unsafe_allow_html=True)
            else: st.info("No players available for head-to-head comparison.")
        st.divider()

        with st.expander(f"🏛️ {clean_display_name} Hall of Fame Archives", expanded=False):
          try: archives_res = supabase.table("archived_seasons").select("season_label, standings_json, archived_at").eq("league_id", selected_league_filter_id).order("archived_at", desc=True).execute().data
          except Exception: archives_res = []
          
          if not archives_res: st.info("No past season archives found yet for this mini-league. Archived seasons will appear here once the commissioner concludes and archives a season!")
          else:
            ...
            if selected_archive_data and selected_archive_data.get("standings_json"):
              ...
              st.subheader(f"📜 {selected_archive_label} Official Season Final Standings — {clean_display_name}"); st.dataframe(pd.DataFrame(formatted_archive_rows), use_container_width=True, hide_index=True)
        st.divider()
        ...
        with st.form(f"trash_talk_form_{selected_league_filter_id}"):
          ...
          if st.form_submit_button("Post Message 💬") and chat_msg.strip():
            if contains_profanity(chat_msg): st.error("⚠️ Your message contains restricted language. Please keep chat friendly!")
            else:
              try: supabase.table("trash_talk").insert({"user_id": user_id, "message": chat_msg.strip(), "league_id": selected_league_filter_id}).execute(); st.success("Message posted!"); st.rerun()
              except Exception as e: st.error(f"Error posting message: {e}")
        ...
        else: st.info("No messages in this mini-league feed yet. Be the first to start the trash talk!")
    st.divider()

    st.subheader("⚙️ Default Standings View"); st.caption("Configure which league standings view automatically displays when you open the Leagues tab.")
    with st.form("settings_league_tab_form"):
      ...
      if st.form_submit_button("Save Default View 💾", type="primary"): supabase.table("profiles").update({"default_league_view": league_s_options[new_def_league_label]}).eq("id", user_id).execute(); st.success("Default standings view updated successfully!"); st.rerun()
    st.divider()
    ...
    with col_create:
      ...
      with st.form("create_league_form"):
        ...
        if st.form_submit_button("Create League 🚀", type="primary"):
          if not new_league_name.strip(): st.error("Please enter a valid league name.")
          elif contains_profanity(new_league_name): st.error("⚠️ League name contains restricted language. Please choose appropriate wording.")
          else:
            import random as r_mod, string as s_mod; invite_code = "".join(r_mod.choices(s_mod.ascii_uppercase + s_mod.digits, k=6))
            try:
              ...
              if res_l.data:
                ...
                st.success(f"League '{new_league_name}' created successfully! Invite Code: **{invite_code}**"); st.rerun()
            except Exception as e: st.error(f"Error creating league: {e}")
    ...
    with col_join:
      ...
      with st.form("join_league_form"):
        ...
        if st.form_submit_button("Join League 🤝", type="primary"):
          ...
          if not clean_code: st.warning("Please enter an invite code.")
          else:
            found_league = supabase.table("leagues").select("id, league_name, league_password").eq("invite_code", clean_code).execute().data
            if not found_league: st.error("Invalid invite code. Please check with your league commissioner.")
            else:
              target_league = found_league[0]
              if target_league.get("league_password", "") and target_league.get("league_password", "") != pwd_input.strip(): st.error("Incorrect league password. Please check with the commissioner.")
              elif supabase.table("league_members").select("id").eq("league_id", target_league["id"]).eq("user_id", user_id).execute().data: st.warning(f"You are already a member of '{target_league['league_name']}'!")
              else: supabase.table("league_members").insert({"league_id": target_league["id"], "user_id": user_id}).execute(); st.success(f"Successfully joined '{target_league['league_name']}'!"); st.rerun()

    st.write(""); st.markdown("#### 📋 Your Joined Mini-Leagues")
  ...
  with tab_settings:
    st.header("⚙️ Account & App Settings"); st.caption("Manage your account security, notification preferences, and accessibility options."); st.write("")
    st.subheader("🔐 Account Security")
    with st.form("settings_password_form"):
      st.markdown("##### Request Password Reset Email"); st.caption("Send a secure password reset link directly to your registered email address.")
      if st.form_submit_button("Send Password Reset Email 🔑", type="primary"):
        try:
          ...
          if response and hasattr(response, "properties") and response.properties:
            ...
            recovery_link = f"https://tdtokens.co.uk/?token={email_otp}&type=recovery" if email_otp else (f"https://tdtokens.co.uk/?token={action_link.split('token=')[1].split('&')[0]}&type=recovery" if action_link and "token=" in action_link else action_link)
            if recovery_link:
              html_content = f"""<div style="background-color: #0b0f19; padding: 30px; font-family: 'Inter', Arial, sans-serif; color: #f8fafc;"><div style="max-width: 600px; margin: 0 auto; background: rgba(15, 23, 42, 0.95); border: 1px solid rgba(255, 255, 255, 0.12); border-top: 4px solid #fbbf24; border-radius: 16px; padding: 40px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);"><div style="text-align: center; margin-bottom: 30px;"><img src="https://github.com/eddymck98/TD-Tokens-Render-/blob/main/TD%20Tokens%207.png?raw=true" alt="Touchdown Tokens Logo" style="width: 180px; margin-bottom: 15px; filter: drop-shadow(0px 6px 15px rgba(251, 191, 36, 0.4));" /><h1 style="font-family: 'Bebas Neue', Arial, sans-serif; color: #fbbf24; font-size: 32px; letter-spacing: 2px; margin: 0;">TOUCHDOWN TOKENS</h1><p style="color: #93c5fd; font-size: 14px; letter-spacing: 3px; text-transform: uppercase; margin-top: 5px;">Password Reset Request</p></div><h3 style="color: #ffffff; font-size: 20px; margin-bottom: 15px;">Reset Your Password 🔑</h3><p style="color: #cbd5e1; font-size: 15px; line-height: 1.6; margin-bottom: 25px;">We received a request to reset your Touchdown Tokens password from your account settings. Click the button below to choose a new password:</p><div style="text-align: center; margin: 35px 0;"><a href="{recovery_link}" style="background: linear-gradient(135deg, #fbbf24 0%, #d97706 100%); color: #000000; padding: 14px 28px; text-decoration: none; border-radius: 12px; font-weight: bold; font-size: 16px; letter-spacing: 1px; display: inline-block; box-shadow: 0 6px 20px rgba(251, 191, 36, 0.3);">RESET PASSWORD</a></div><p style="color: #94a3b8; font-size: 13px; line-height: 1.5; margin-top: 30px; border-top: 1px solid rgba(255, 255, 255, 0.08); padding-top: 20px;">If you did not request a password reset, you can safely ignore this email.</p></div></div>"""
              resend.Emails.send({"from": "Touchdown Tokens <noreply@auth.tdtokens.co.uk>", "to": [st.session_state.user.email], "subject": "🔑 Reset Your Touchdown Tokens Password", "html": html_content}); st.success("Password reset email sent to your inbox!")
            else: st.error("Could not retrieve recovery link properties from the response.")
          else: st.error("Could not generate recovery link for this email.")
        except Exception as e: st.error(f"Error sending password reset email: {e}")

    with st.form("settings_email_form"):
      st.markdown("##### Change Registered Email"); st.caption(f"Current Email: `{st.session_state.user.email}`")
      new_email_input = st.text_input("New Email Address", key="settings_new_email")
      if st.form_submit_button("Update Email ✉️"):
        if not new_email_input.strip() or "@" not in new_email_input: st.warning("Please enter a valid email address.")
        else:
          try:
            supabase.auth.update_user({"email": new_email_input.strip()}); supabase.table("profiles").update({"email": new_email_input.strip()}).eq("id", user_id).execute()
            st.success("Email update requested! Check your new email inbox for confirmation.")
          except Exception as e: st.error(f"Error updating email: {e}")

    st.divider(); st.subheader("🔔 Notification Preferences")
    with st.form("settings_notifications_form"):
      ...
      if st.form_submit_button("Save Notification Settings 💾"): supabase.table("profiles").update({"email_notifications": new_notif}).eq("id", user_id).execute(); st.success("Notification preferences saved!"); st.rerun()

    st.divider(); st.subheader("♿ Accessibility & Display Preferences")
    with st.form("settings_accessibility_form"):
      ...
      if st.form_submit_button("Save Accessibility Settings 💾"): supabase.table("profiles").update({"high_contrast_mode": new_hc, "reduced_motion": new_rm}).eq("id", user_id).execute(); st.success("Accessibility preferences saved!"); st.rerun()

    st.divider(); st.subheader("🚪 Session Management"); st.caption("Log out of your account securely from this device.")
    if st.button("Log Out of Account 🚪", type="secondary"):
      try: supabase.auth.sign_out()
      except Exception: pass
      controller.remove("td_tokens_session"); st.session_state.user = None
      if "supabase_client" in st.session_state: del st.session_state["supabase_client"]
      st.rerun()

  if is_any_league_admin:
    with tab_league_admin:
      st.markdown("## ⭐ League Commissioner Administration"); st.caption("Manage your mini-leagues, update settings, regenerate invite codes, and handle member rosters."); st.write("")
      ...
      if not admin_leagues_list: st.info("You are not currently designated as a commissioner for any custom mini-leagues.")
      else:
        ...
        st.write(""); col_m1, col_m2, col_m3 = st.columns(3)
        with col_m1: st.metric("League Name", l_name)
        with col_m2: st.metric("Invite Code", l_code)
        with col_m3: st.metric("Total Members", str(len(members_res) if members_res else 0))
        st.write("")
        ...
        with st.expander("📝 1. League Settings & Security", expanded=True):
          with st.form(f"league_settings_form_{l_id}"):
            ...
            if st.form_submit_button("Save League Settings 💾", type="primary"):
              if not new_l_name.strip(): st.error("League name cannot be blank.")
              elif contains_profanity(new_l_name): st.error("⚠️ League name contains restricted language. Please choose appropriate wording.")
              else:
                try:
                  ...
                  st.cache_data.clear(); st.success("League settings updated successfully!"); st.rerun()
                except Exception as e: st.error(f"Error updating league settings: {e}")
            ...
            if member_names_map:
              with st.form(f"kick_form_{l_id}"):
                ...
                if st.form_submit_button("Remove Member from League 🚪", type="secondary"):
                  if not confirm_kick: st.warning("Please check the confirmation box to remove the selected member.")
                  else:
                    try:
                      supabase.table("league_members").delete().eq("league_id", l_id).eq("user_id", member_names_map[target_kick_name]).execute()
                      st.cache_data.clear(); st.success(f"Successfully removed {target_kick_name} from {l_name}."); st.rerun()
                    except Exception as e: st.error(f"Error removing member: {e}")
            else: st.info("No other members in this league to remove.")
        ...
        with st.expander("👑 3. Conclude Season & Crown Champion", expanded=False):
          ...
          with st.form(f"conclude_season_form_{l_id}"):
            ...
            if st.form_submit_button("Crown Champion & Archive Season 🏆", type="primary"):
              if not confirm_conclude: st.warning("Please check the confirmation box to proceed.")
              else:
                try:
                  ...
                  st.cache_data.clear(); st.balloons(); st.success(f"Successfully concluded '{season_label_input}' for {l_name}! Champion crowned and archived to Hall of Fame."); st.rerun()
                except Exception as e: st.error(f"Error concluding season: {e}")

        with st.expander("🔑 4. Invite Code & Ownership Tools", expanded=False):
          col_tc1, col_tc2 = st.columns(2)
          with col_tc1:
            st.markdown("##### Regenerate Invite Code"); st.caption("Generate a brand new 6-character code, invalidating the old one.")
            if st.button("Generate New Invite Code 🔑", key=f"btn_regen_{l_id}"):
              import random as r_m, string as s_m; new_code = "".join(r_m.choices(s_m.ascii_uppercase + s_m.digits, k=6))
              supabase.table("leagues").update({"invite_code": new_code}).eq("id", l_id).execute(); st.cache_data.clear(); st.success(f"New invite code generated: **{new_code}**"); st.rerun()
          with col_tc2:
            st.markdown("##### Transfer Ownership"); st.caption("Transfer commissioner rights to another member.")
              ...
              with st.form(f"transfer_ownership_form_{l_id}"):
                ...
                if st.form_submit_button("Transfer Ownership 👑"):
                  try:
                    ...
                    st.cache_data.clear(); st.success(f"Successfully transferred commissioner ownership to {new_owner_name}!"); st.rerun()
                  except Exception as e: st.error(f"Error transferring ownership: {e}")
            else: st.info("No other members available for transfer.")
  ...
  if profile.get("is_admin"):
    with tab_admin:
      ...
      if admin_sec == "Manage Questions":
        ...
        with col_w_sel: selected_manage_week = st.selectbox("Select Week to Manage", week_options, index=len(week_options) - 1, key="admin_manage_week_sel")
        with col_w_btn1:
          st.write("")
          if st.button("📋 Load Templates"):
            ...
            st.success("Templates loaded!"); st.rerun()
        with col_w_btn2:
          st.write("")
          if st.button("🗑️ Clear Week"):
            try:
              ...
              for i in range(1, 11):
                if f"m_prompt_w{selected_manage_week}_q{i}" in st.session_state: del st.session_state[f"m_prompt_w{selected_manage_week}_q{i}"]
              st.cache_data.clear(); st.success(f"Cleared Week {selected_manage_week}!"); st.rerun()
            except Exception as e: st.error(f"Error: {e}")
        ...
        with st.form(key=f"manage_questions_form_week_{selected_manage_week}"):
          ...
          for i in range(1, 11):
            ...
            with col_m1: away_t = st.selectbox(f"Away Team (Q{i})", NFL_TEAMS, index=(NFL_TEAMS.index(existing_away) if existing_away in NFL_TEAMS else 0), key=f"m_away_w{selected_manage_week}_q{i}")
            with col_m2: home_t = st.selectbox(f"Home Team (Q{i})", NFL_TEAMS, index=(NFL_TEAMS.index(existing_home) if existing_home in NFL_TEAMS else 0), key=f"m_home_w{selected_manage_week}_q{i}")
          ...
          if st.form_submit_button("Save & Publish All Questions 💾", type="primary"):
            if any(contains_profanity(item["prompt"]) for item in question_payloads): st.error("⚠️ One or more question prompts contain restricted language. Please check your wording.")
            else:
              try:
                for item in question_payloads:
                  combined_text = f"{item['prompt']} | MATCHUP: {item['away']} @ {item['home']}"
                  if item["db_id"]: supabase.table("weekly_questions").update({"question_text": combined_text}).eq("id", item["db_id"]).execute()
                  else: supabase.table("weekly_questions").insert({"week_number": selected_manage_week, "question_number": item["question_number"], "question_text": combined_text, "winning_answer": "Pending"}).execute()
                st.cache_data.clear(); st.success(f"Successfully published all 10 questions for Week {selected_manage_week}!"); st.rerun()
              except Exception as e: st.error(f"Error publishing questions: {e}")
        ...
        if not sched_week_nums: st.info("No weeks found in the database. Create questions first.")
        else:
          ...
          with st.form("lockout_scheduler_form"):
            ...
            with col_d: lock_date = st.date_input("Lockout Date (Sunday Kickoff)", value=default_dt.date())
            with col_t: lock_time = st.time_input("Lockout Time (UTC)", value=default_dt.time())
            st.write(""); col_btn_set, col_btn_clear = st.columns(2)
            with col_btn_set: submit_lock = st.form_submit_button("Save Lockout Schedule ⏳", type="primary")
            with col_btn_clear: clear_lock = st.form_submit_button("Clear Lockout Timer 🗑️", type="secondary")
            
            if clear_lock:
              try:
                supabase.table("weekly_questions").delete().eq("week_number", sel_sched_week).ilike("winning_answer", "LOCKTIME:%").execute()
                st.cache_data.clear(); st.success(f"Lockout timer cleared for Week {sel_sched_week}!"); st.rerun()
              except Exception as e: st.error(f"Error clearing lockout: {e}")
            ...
            if submit_lock:
              ...
              try:
                ...
                st.cache_data.clear(); st.success(f"Successfully scheduled automatic lockout for Week {sel_sched_week} at {iso_str} UTC!"); st.rerun()
              except Exception as e: st.error(f"Error saving lockout: {e}")
        ...
        if not grading_week_nums: st.info("No weeks available to grade.")
        else:
          ...
          if not real_grade_qs: st.warning("No questions found for this week to grade.")
          else:
            with st.form("live_grading_form"):
              winning_answers_dict = {}; st.markdown("<h4 style='color: #fff; font-family: Bebas Neue; letter-spacing: 1px;'>Live Matchup Results</h4>", unsafe_allow_html=True)
              for q in real_grade_qs:
                ...
                if curr_ans not in ["Yes", "No"]: curr_ans = "Pending"
              ...
              st.write(""); st.markdown("<h4 style='color: #fff; font-family: Bebas Neue; letter-spacing: 1px;'>Touchdown Scorer Bonus Picks</h4>", unsafe_allow_html=True)
              ...
              else: st.info("No touchdown scorer picks submitted for this week.")
              
              st.write(""); col_grd1, col_grd2 = st.columns(2)
              with col_grd1: submit_grading = st.form_submit_button("Save Live Results & Recalculate Tokens 🚀", type="primary")
              with col_grd2: manual_close_week = st.form_submit_button("Manually Close Week 🔒", type="secondary")
              
              if submit_grading:
                try:
                  for q_id, chosen_ans in winning_answers_dict.items(): supabase.table("weekly_questions").update({"winning_answer": chosen_ans}).eq("id", q_id).execute()
                  if week_td_picks:
                    for td_id, grade_val in td_grading_dict.items(): supabase.table("touchdown_picks").update({"is_correct": True if grade_val == "Correct (+5 🪙)" else (False if grade_val == "Incorrect" else None)}).eq("id", td_id).execute()
                  recalculate_all_user_balances(supabase); st.cache_data.clear(); st.success(f"Successfully updated live results for Week {grade_week_sel} and instantly recalculated all player token balances!"); st.rerun()
                except Exception as e: st.error(f"Error grading week live: {e}")
              ...
              if manual_close_week:
                try:
                  ...
                  st.cache_data.clear(); st.success(f"Week {grade_week_sel} has been manually closed!"); st.rerun()
                except Exception as e: st.error(f"Error closing week: {e}")
        ...
        if not all_profiles: st.info("No players found.")
        else:
          ...
          with st.form("bulk_token_form"):
            ...
            if st.form_submit_button("Apply Token Adjustment 💾", type="primary"):
              if token_amount_input == 0: st.warning("Please enter a token amount greater than 0.")
              else:
                ...
                try:
                  ...
                  st.cache_data.clear(); st.success(f"Successfully updated token balance for {sel_p_name} to {new_tokens} 🪙!"); st.rerun()
                except Exception as e: st.error(f"Error adjusting tokens: {e}")
        ...
        if not all_profiles_export: st.info("No profile data available for export.")
        else:
        ...
        with st.form("admin_announcement_form"):
          ...
          if st.form_submit_button("Broadcast Announcement 📢", type="primary"):
            if not announcement_text.strip(): st.warning("Announcement message cannot be blank.")
            elif contains_profanity(announcement_text): st.error("⚠️ Announcement contains restricted language. Please revise.")
            else:
              try: supabase.table("trash_talk").insert({"user_id": user_id, "message": f"🚨 **COMMISSIONER ANNOUNCEMENT:** {announcement_text.strip()}", "league_id": "00000000-0000-0000-0000-000000000001"}).execute(); st.success("Official commissioner announcement broadcast successfully!")
              except Exception as e: st.error(f"Error broadcasting announcement: {e}")
        ...
        with st.form("global_season_archive_form"):
          ...
          if st.form_submit_button("Crown Global Champion & Reset Season 🏆", type="primary"):
            if not confirm_global_reset: st.warning("Please check the confirmation box to proceed.")
            else:
              try:
                ...
                for p_item in sorted_global_players: supabase.table("profiles").update({"tokens": 10}).eq("id", p_item["id"]).execute()
                st.cache_data.clear(); st.balloons(); st.success(f"Successfully archived '{season_title_input}'! Global Champion crowned and all player token banks reset to 10 tokens."); st.rerun()
              except Exception as e: st.error(f"Error archiving global season: {e}")
        ...
        with st.form("app_access_control_form"):
          ...
          if st.form_submit_button("Save Access Control Settings 🔒", type="primary"):
            ...
            st.cache_data.clear(); st.success("App access controls successfully updated!"); st.rerun()
